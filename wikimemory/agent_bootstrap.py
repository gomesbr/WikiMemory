...
def render_agent_bootstrap(items: list[dict[str, object]], renderer: str) -> tuple[str, int]:
    if renderer == "codex_agents_md":
        return render_bootstrap_markdown(items, "Codex")
    if renderer == "claude_md":
        return render_bootstrap_markdown(items, "Claude")
    if renderer == "generic_bootstrap_md":
        return render_bootstrap_markdown(items, "agent")
...
def render_bootstrap_markdown(items: list[dict[str, object]], agent_label: str) -> tuple[str, int]:
    ...
    for project in projects[:8]:
        lines.append(f"   -> ./memory/projects/{project}/project.md")
    lines.extend(["", "3. Recent context (highest priority):"])
    for project in projects[:8]:
        lines.append(f"   -> ./memory/projects/{project}/recent.md")
    lines.extend(["", "4. Project rules:"])
    for project in projects[:8]:
        lines.append(f"   -> ./memory/projects/{project}/rules.md")
