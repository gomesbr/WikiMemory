...
def render_agent_bootstrap(items: list[dict[str, object]], renderer: str) -> tuple[str, int]:
    if renderer == "codex_agents_md":
        return render_bootstrap_markdown(items, "# Agent Bootstrap", "Codex")
    if renderer == "claude_md":
        return render_bootstrap_markdown(items, "# Claude Memory Bootstrap", "Claude")
...
def render_bootstrap_markdown(items: list[dict[str, object]], title: str, agent_label: str) -> tuple[str, int]:
    ...
    lines = [
        title,
        "",
        f"This file is generated by WikiMemory from compact derived memory. Treat linked memory files as the source of truth for {agent_label} context.",
        "",
        "## Read First",
        "",
